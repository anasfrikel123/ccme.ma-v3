---
title: "Comptable textile et auto Tanger | Zones franches"
h1: "Comptable industrie textile et automobile"
description: "Cabinet spécialisé pour l'industrie textile et l'équipementier automobile à Tanger Free Zone, TAC, Tanger Tech. Régime ZAI."
eyebrow: "Secteurs"
keywords:
  - "comptable textile Tanger"
  - "comptable automobile Tanger"
  - "expert comptable TFZ"
  - "expert comptable TAC"
heroImage: "/images/secteurs/textile-automobile.jpg"
bullets:
  - "Régime fiscal ZAI : exonération IS 5 ans"
  - "Comptabilité de production industrielle"
  - "Stock matières / encours / produits finis"
  - "Conventions sociales sectorielles"
  - "Prix de transfert avec maisons-mères"
faq:
  - q: "Comment fonctionne l'exonération IS en zone franche ?"
    a: "Exonération totale d'IS pendant les 5 premiers exercices. Au-delà, taux réduit de 8,75% sur les opérations export. Conditions : agrément ZAI, implantation effective, activité éligible. Beaucoup d'industriels textiles et automobiles à TFZ et TAC en bénéficient."
---

Les zones franches de Tanger ([Tanger Free Zone](/zones/tanger-free-zone), [Tanger Automotive City](/zones/tanger-automotive-city), [Tanger Tech](/zones/tanger-tech)) accueillent une concentration unique d'industriels — équipementiers automobiles (Renault, Stellantis, Yazaki, Sumitomo), textile-confection, électronique, aéronautique.

Pour ces structures, la compta a des spécificités importantes :

## Le régime fiscal de faveur

Exonération [IS](/outils/simulateur-is) pendant 5 exercices, puis taux réduit (8,75% sur l'export). Exonération de [TVA](/outils/simulateur-tva) à l'importation des biens d'équipement. Régime douanier avantageux. Mais ces avantages sont conditionnels :

- Agrément ZAI préalable, dossier monté en amont avec le [CRI](/glossaire#cri) Tanger
- Activité éligible (industrie d'export essentiellement)
- Implantation physique effective dans la zone
- Suivi distinct des opérations zone vs hors zone

Une mauvaise délimitation, c'est la perte rétroactive du régime de faveur. Sanction lourde.

## Comptabilité industrielle

Pas comme une PME de services. Stock matières premières, en-cours de production, produits finis. Comptabilité analytique par produit, calcul de coût de revient, suivi des écarts.

Pour les automotive en JIT (just-in-time), il faut des reportings très fins — souvent quotidiens — pour le donneur d'ordre.

## Prix de transfert

La quasi-totalité des industriels en zone franche sont des filiales de groupes internationaux. Les transactions intra-groupe (royalties, frais de support, fournitures) doivent être documentées (master file + local file), à des prix conformes au principe de pleine concurrence. Sinon redressement avec pénalités.

On accompagne plusieurs industriels à TFZ et TAC. Les sujets récurrents : le drawback douanier quand une partie de la matière importée finit en vente locale sans qu'on ait isolé la quote-part, les remboursements TVA sur achats locaux de matières premières (dossiers lents, souvent bloqués sur des formalités de justification que peu de comptables maîtrisent), et la base cotisable [CNSS](/glossaire#cnss) pour les ouvriers de production en équipes alternantes — primes de poste et d'assiduité que beaucoup de services RH intègrent mal dans l'assiette, ce qui génère des redressements à répétition.
